...
class DbHelpers implements DbHelpersContract {
	...
	public function get_record_id( $post_id, $activity_id, $video_id = '', $user_id = 0 ) {
		...
		$sql_query = $wpdb->prepare(
			"
				...
				WHERE user_id = %d AND post_id = %d AND activity_id = %d
			", $user_id, $post_id, $activity_id
		);

		if ( $video_id ) {
			$sql_query .= $wpdb->prepare( ' AND video_id = %s', $video_id );
		}
	...
	public function get_all_records( $user_id = 0 ) {
		...
		$sql_query = $wpdb->prepare(
			"
				...
				WHERE user_id = %d
				ORDER BY CASE 
						WHEN date_updated = '0000-00-00 00:00:00' THEN date_created
						ELSE date_updated
					END
				DESC
			", $user_id
	...
	public function get_records_by_activity( $activity_id, $user_id = 0, $limit = 0 ) {
		...
		$sql_query .= $this->get_paged_sql( $limit );
	 ...
	 * @return string
	 */
	protected function get_paged_sql( $limit = 0 ) {
		$per_page = $limit ?: $this->config->per_page;
