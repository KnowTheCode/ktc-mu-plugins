...
function add_technology_to_breadcrumb( $html, array $args ) {
	...
	$name       = get_technology_name_by_slug( $technology );
	if ( ! $name ) {
		return $html;
	}
	...
	$html = insert_technology_into_breadcrumb( $html, $technology_html, $args['sep'] );
	return change_docx_link_in_breadcrumb( $html );
}
