 ...
 *
 * @wordpress-plugin
 * Plugin Name:     KTC Partners Plugin
 * Plugin URI:      https://knowthecode.io
 * Description:     Our sponsoring partners get some love with this plugin - as the sponsors custom post type, level taxonomy, custom fields, shortcodes, and templates 
 * Version:         1.0.2
 * Author:          hellofromTonya
...
fulcrum_declare_plugin_constants( 'PARTNERS', __FILE__ );
...
function launch( Fulcrum_Contract $fulcrum ) {
	...
	$fulcrum['partners'] = $instance = new Controller(
		new Config( $path ),
		__FILE__,
		$fulcrum
	);
...
register_activation_hook( __FILE__, function() {
	...
	if ( get_option( 'partner_adspot_next', false ) === false ) {
		update_option( 'partner_adspot_next', 0, true );
	}
